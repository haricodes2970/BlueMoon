...
Docker was chosen over a zero-config buildpack for this
pnpm/Turborepo workspace shape, and
[ADR-0033](./docs/adr/ADR-0033-adopt-render-for-backend-hosting.md)
for the platform decision itself (Render, superseding an earlier
Railway choice — changed 2026-08-16 before any external deployment
happened; the Dockerfile and every runtime behavior verified below
were unaffected by the switch). `apps/web` needs no `vercel.json` —
Vercel's native Next.js monorepo support handles it via a Root
Directory setting alone. Verified locally (2026-08-15, re-verified
2026-08-16 after the Render switch): `docker build` succeeds; the
built image starts against a real disposable PostgreSQL instance with
production-shaped environment variables (`NODE_ENV=production`, a
non-default `WEB_ORIGIN`, etc.) and passes `/health`; a full golden
...
behavior). **No actual Vercel or Render deployment has
been performed** — see [docs/deployment/README.md](./docs/deployment/README.md)'s
Milestone 1.0 Completion Criteria for the exact repository-ready vs.
...
**Milestone 1.0 — Platform Change: Railway → Render (2026-08-16)**

- [x] Platform target changed to Render before any external Railway
      (or Render) deployment was ever performed —
      [ADR-0033](./docs/adr/ADR-0033-adopt-render-for-backend-hosting.md)
      supersedes ADR-0012; ADR-0031 and ADR-0032 amended in place
      (status-line notes only, bodies unchanged) since their reasoning
      — proxy-trust, cross-origin cookies, Docker-over-buildpack — is
      unaffected by which platform sits behind the Dockerfile
- [x] `railway.json` removed; `render.yaml` Blueprint added (web
      service + PostgreSQL in one file; `DATABASE_URL` wired to
      Render's internal connection string via `fromDatabase`;
      `JWT_ACCESS_TOKEN_SECRET` generated by Render via
      `generateValue: true`, never handled by a human)
- [x] Code comments naming "Railway's edge" as the trusted single
      reverse-proxy hop updated to "Render's edge"
      (`infrastructure/identity/client-ip.ts` + its test, `env.ts`,
      `index.ts`) — the assumption itself (exactly one trusted hop)
      and the code implementing it are unchanged
- [x] `docs/deployment/README.md` rewritten Railway → Render
      throughout (topology, environment variable tables, Docker Build
      section, health check, rollback, domain-shape examples)
- [x] Zero functional/runtime code changes — confirmed via unchanged
      `pnpm test` (81/81) / `pnpm test:db` (67/67) and a re-verified
      `docker build`/`docker run` against the same unmodified
      Dockerfile
- [x] **No actual Render or Vercel deployment performed** — this pass
      is repository preparation only; see
      `docs/deployment/README.md`'s Milestone 1.0 Completion Criteria
- [x] Docs updated: `docs/deployment/README.md`, ADR-0033 (new),
      ADR-0012/ADR-0031/ADR-0032 (amended), this file, `ROADMAP.md`,
...
- Messaging rate limiting, WebSocket heartbeat, and cookie/CORS
  ...
  [ADR-0033](./docs/adr/ADR-0033-adopt-render-for-backend-hosting.md)
  — the platform target changed from Railway to Render 2026-08-16
  before any external deployment happened) — **no actual Vercel or
  Render deployment has been performed**;
  [docs/deployment/README.md](./docs/deployment/README.md)'s
  Milestone 1.0 Completion Criteria section is the authoritative
